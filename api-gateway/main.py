...
async def proxy(path: str, request: Request):
    ...
    if not base:
        return JSONResponse(
            status_code=404,
            content={"detail": "Unknown service route"}
        )

    method = request.method
    query = str(request.url.query)

    # Clear product cache when products are modified.
    if first == "products" and method in {"POST", "PATCH", "DELETE", "PUT"}:
        _product_cache.update({"time": 0, "data": None, "query": None})

    # Product list cache for sustainability/performance discussion.
    if method == "GET" and path == "products":
        now = time.time()
        if (
            _product_cache["data"] is not None
            and _product_cache["query"] == query
            and now - _product_cache["time"] < CACHE_SECONDS
        ):
            return JSONResponse(
                content=_product_cache["data"],
                headers={"X-GlowCart-Cache": "HIT"}
            )
